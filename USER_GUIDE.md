...
**Credit Card Authorization Form**: on any existing client's record, right below Attachments, click **🔒 Copy Upload Link** — this copies a personalized link to your clipboard that you send the client by email or text. They open it, upload their completed and signed credit card authorization PDF, and it's securely sent straight to a private Google Drive folder — it's never emailed, never passes through this app, and never sits in the client's browser. The client doesn't need any account or password to use it. Once they've uploaded it, you'll find the file in your Drive under **Miles Man → Credit Card Authorization Forms** (or via "Browse Google Drive" on their record) — attach it to their record the normal way once you've reviewed it. The button only appears once the client's been saved at least once — a brand-new, not-yet-saved record has no client ID yet for the link to carry.
...
- **Email** (the mail-merge tool) — quick canned messages: Welcome New Client, General Follow-Up, Trip Confirmation, Thank You & Referral, **Birthday Trip Planning**, or **Referral Credit Earned**. Pick a client, preview with their name/email filled in, send.
