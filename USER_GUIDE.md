...
Once linked, the Trips page shows those legs grouped together under one summary line (**Trip Name** if you've set one on any of the legs, client name, every leg's destination in order, the overall date range, and a trip count) instead of scattered among all your other unrelated trips — expanding it reveals each leg as its own indented row rather than just the one trip's own row. Search, status filter, and column sorting all still work as usual — a group just moves as a unit to wherever its first leg lands under the current sort.
