...
**The public form's Hotel, Theme Park, and Tours sections can each hold more than one entry now** — a "+ Add Another Hotel" / "+ Add Another Theme Park" / "+ Add Another Tour" button under each one lets a client request, say, two different hotel stays or three separate tours in the same submission. Every entry a client adds shows up as its own line in the resulting trip's Notes (e.g. "Hotel 1: ...", "Hotel 2: ..."), and each hotel brand/theme park/tour vendor named still gets checked against your Suppliers the same way a single entry always did.
