...
Every number in the row of boxes at the top of Dashboard, Clients, Trips, Suppliers, Vendors, and Planner can be clicked. It opens a list of exactly which records add up to that number — click any row in that list to jump straight to that client, trip, supplier, vendor, or task. This is the fastest way to answer "which trips make up this revenue number" or "who are these upcoming clients." **Click any column header in that list to sort by it** — once for ascending, again for descending — so you can, say, sort a Total Revenue breakdown by dollar amount to find your biggest bookings at a glance.
...
This page (labeled **Individual Clients** in the sidebar, right above **Corporate**) is for individual travelers, exactly as before — the rename is just to distinguish it from the separate [Corporate](#corporate) section for company accounts.
...
**A companion traveler (the "Other Travelers" section of the form, once Number of Travelers is more than 1) gets the exact same fields and the exact same enrichment behavior as the primary submitter** — Email, Phone, Address, Referred By, Discounts, and Interests included, not just birthdate/citizenship/preferences/points balances. There's one real limitation worth knowing: a companion traveler has no email on the form, so matching them to an existing client is by name only — and the form's "Name as shown on passport or license" prompt means someone might type "Jacqueline B Rogers" when your existing record is under a nickname like "Jakki Rogers." Rather than risk silently creating a duplicate, a companion traveler whose name doesn't exactly match anyone but who **shares a last name with an existing client** gets held for your review instead of auto-created — sidebar → Import → **Pending Traveler Matches**, which lists each one with a hint of who it might be, a dropdown of every client to pick the right match (or **+ Add as New Client** if it really is someone new, or **Discard** to drop it). A traveler with no exact match and nobody sharing their last name either is still added automatically as before — this review step only kicks in when there's an actual name to double-check. **A family member on the same booking never counts as an ambiguous match for another family member on that same booking** — sharing a last name with the person who submitted the form, or with a sibling on the same submission, is completely normal, and used to needlessly send several members of the very same family to your review queue every time (and, if the wrong one got picked from that queue's dropdown, could even merge one traveler's details onto a completely different family member's record). Only a genuinely separate, unrelated client sharing a last name still gets flagged.

**Each additional traveler on the public form has a "Same address as Traveler 1" checkbox** above their address fields — for a family or group traveling together, this saves them from retyping the same street address, city, state, and zip for everyone. Checking it fills those four fields from the primary contact's own address and grays them out; unchecking lets them edit that traveler's address independently again.

**The public form only collects full details for up to 4 travelers at a time (you plus 3 more)** — with more travelers than that in a submission, so many fields get sent at once (a full balance grid × every traveler) that Formspark was silently dropping data before your notification email even went out, so a client with 6+ travelers was quietly missing people. Once "Number of Travelers" goes above 4, the form shows a note saying anyone beyond the 4th will be collected separately, and after they submit, the thank-you message includes a link to a follow-up page for the rest of their group — it can be re-used again and again for a party of any size, 4 travelers at a time. **The notification email for a Book a Trip submission (or its additional-travelers follow-up) now only lists the fields that were actually filled in** — a blank balance field or an untouched "-- Select --" dropdown no longer shows up as its own empty line, so the email reads as a real list of what someone actually told you rather than pages of "Field: (nothing)."

That follow-up submission arrives by email the same way and **is imported automatically, straight onto the trip the original submission created** — it's matched to that trip by the primary contact's name/email (whoever it's "continuing the request for"), then each additional traveler is added to that trip exactly like a companion traveler on the original submission would be: matched to an existing client, created fresh, or — if their name is ambiguous — queued to Pending Traveler Matches for you to resolve, same as always. Nothing needs to be added by hand unless a match ends up in that queue.

**The public form's Hotel, Theme Park, and Tours sections can each hold more than one entry now** — a "+ Add Another Hotel" / "+ Add Another Theme Park" / "+ Add Another Tour" button under each one lets a client request, say, two different hotel stays or three separate tours in the same submission. Every entry a client adds shows up as its own line in the resulting trip's Notes (e.g. "Hotel 1: ...", "Hotel 2: ..."), and each hotel brand/theme park/tour vendor named still gets checked against your Suppliers the same way a single entry always did. **Every entry also lands directly in the new trip's Booked Trip Details** — open the trip and its Hotel/Theme Park/Tours sections are already pre-filled with what the client asked for, ready for you to add the confirmation number and price once it's actually booked, instead of you having to retype everything by reading the Notes text.
...
A directory of corporate clients (company accounts, as opposed to an individual traveler on the [Individual Clients](#individual-clients) page) — same shape as Vendors/Communities, just an Industry select instead of Town/County. Each Corporate Client has a Company Name, Industry (Law Firm, Real Estate, Finance/Banking, Healthcare, Technology, Manufacturing, Retail, Construction, Insurance, Nonprofit, Government, Other), Phone, Website, and Notes.

**Contacts work exactly like Vendor/Community Contacts** — open a Corporate Client and scroll to its own **Contacts** section: **+ Add Contact** opens that person's own record (typically the company's HR/travel coordinator or an executive assistant) with a filterable Notes Log and a Tasks list with **+ Add Task**. Every contact is also clickable right on the plain Corporate list page, each one listed under its company's card.

**Planner tasks can link to a Corporate Contact** the same way they link to a Supplier, Vendor, or Community Contact — the task form has its own **Corporate Contact** dropdown, and picking one auto-fills that contact's phone/email and shows a **Corporate** badge (indigo) on the task.
...
2. **Booked Trip Details** — starts with an optional **Trip Name** (e.g. "Smith Family Alaska Cruise") — purely a friendly label, used only to personalize the greeting line of the Email Itinerary ("Here's your itinerary for Smith Family Alaska Cruise!") instead of the plain destination. Below that, a more detailed, confirmed-booking recap broken into the same categories as the public trip-request form: **Airline, Hotel, Cruise, Car Rental, Tours, Theme Park, Train, Bus.** Each one has its own **confirmation number**, **Price ($)**, and **Notes** field (Tours uses its existing Description field for this instead of a separate Notes box), so you're not hunting through the trip's main Notes box for a locator code, a cost, or a detail specific to that one piece of the booking.
   ...
   - **Hotel** supports **more than one hotel per trip** — click **+ Add Hotel** for each additional stay (useful for multi-city trips). Each hotel entry gets its own confirmation number, check-in date, number of nights, price, notes, and its own Points Used list (add as many redemption rows as that stay needs). Remove a hotel entry with the trash icon on that row.
     - Each hotel entry also has a **🧾 Net Rate Voucher** button. For a stay you've booked at a net/wholesale rate, click it to generate a one-page voucher (with the confirmation number, guest name(s), check-in/check-out dates, and a note asking the hotel not to disclose the rate to the guest) — hand it to the client to bring to the front desk. It downloads as a PDF and also copies to your clipboard so you can paste it straight into an email. You don't need to save the trip first — it uses whatever's currently filled in on that hotel entry and whichever client(s) are selected above.
   ...
   - **Theme Park** likewise now supports **more than one park per trip** — click **+ Add Theme Park** for each additional park/ticket purchase. Each entry gets its own park, confirmation number, ticket type, price, and notes; Insurance Premium/Coverage and Points Used stay shared across the whole Theme Park section (not per park) since one policy or points redemption usually covers the whole ticket purchase. Remove a theme park entry with the trash icon on that row.
...
**Revenue imports set the trip's status from the travel date itself**, not a one-size-fits-all label — a trip whose travel date hasn't happened yet comes in as **Upcoming**, one happening today as **In Progress**, and one already past as **Completed**. (An earlier version used a "Confirmed" placeholder for anything still upcoming; already-imported trips still showing that got corrected automatically to the right one of the three above.)
...
**Disney and Universal are now Hotel suppliers with their official on-site properties pre-loaded**, alongside Marriott/Hilton/Hyatt/Accor — if you'd already added either one yourself, your existing record (and anything you've customized on it) is left alone; this only fills in the gap if it wasn't already there. Universal's list only covers Universal Orlando Resort (Universal Studios Hollywood has no on-site hotels), including the two newest hotels that opened alongside Epic Universe. Both brands also now show up wherever you'd expect a hotel brand to appear — the Preferred Hotel picker on client records and the Book a Trip form, Hotel Points Balances, and the Hotel Brand/Property pickers on both the trip form's Booked Trip Details and the public Book a Trip form.
...
**Contact Emails** — right below Contacts, a combined feed of every email Outlook has logged across *all* of that supplier's contacts, newest first, so you can see the whole email history with a supplier at a glance instead of opening each contact one by one. Each entry shows who it's with and when; click one to jump straight to that contact's own record if you need to see more or make a change.

**Properties**, for a Hotel-category Supplier, is the searchable list of that brand's individual hotels — the same list you pick from when filling in a Property on a trip's Booked Trip Details. Add or remove property names with the controls right below the list. **Select just one property and a Property Email field appears below it** — type an email there and click **Save Email** to attach it to that specific property (e.g. a particular hotel's own group sales desk), separate from any of the Supplier's own Contacts. A property with an email on file shows a small ✉ next to its name so it stands out while scrolling a long list. Like everything else on this form, saving a property's email doesn't stick until you click **Save Supplier**.

**If a Supplier turns out to be one property of another Supplier, you can move it in.** This comes up when a revenue import auto-creates a Supplier from one hotel's invoice (say "The Ritz-Carlton, Naples") and you later realize it's really one property of a brand you already have ("Marriott"). Open the standalone Supplier's edit form, scroll to **Actually a property of another supplier?** near the bottom, pick the parent Supplier, and click **Move as Property**. The CRM adds this record's name (and its email, if any) as a property on the parent, re-points every trip that pointed at the standalone record to the parent, moves its contacts (tagged to the new property name), notes, and linked clients over, and then deletes the standalone record. If the parent is a recognized hotel brand, affected trips also get their Hotel Brand / Property filled in where those were blank. It's one action on the Undo stack, so Ctrl+Z puts everything back.

**A Contact can also be tied to a specific Property.** If a Supplier is a Hotel with properties on file, its Add/Edit Contact form gets a searchable **Property** field — pick the one hotel this particular person actually works at (their own group sales desk, say), and it shows right in that contact's summary line everywhere it appears (the Contacts list, the Supplier card) so you can tell at a glance which property each contact belongs to instead of opening each one to check.

**You can also go the other way** — from the Properties list itself. Select just one property and a **Property Contacts** panel appears below the Property Email field, listing anyone already tied to that property (click a name to open their record) or a note that no one is yet. A **+ Add Contact for This Property** button opens a brand-new contact form with that property already filled in, so you don't have to hunt for it in the picklist — handy when you're already looking at a property and just realized you need to add its group sales contact.
...
**Planner tasks can link to a Community Contact** the same way they link to a Supplier or Vendor Contact — the task form has its own **Community Contact** dropdown, and picking one auto-fills that contact's phone/email and shows a **Community** badge (teal) on the task. The starter batch above came with one "Call [Community]" task per community already on the Planner, all due today.
...
**The Planner table now has a color-coded Type column** — green **Supplier**, amber **Vendor**, blue **Client** — showing which kind of contact a task is really about, and the Client/Phone/Email columns follow the same priority: a linked Supplier Contact wins first, then a Vendor Contact, then a Linked Supplier with no specific contact picked, then a linked client. So a task about, say, a cruise line's group desk contact or your printer's sales rep shows *their* name/phone/email front and center, not a client's that happens to also be tagged on it.

**If that automatic pick isn't the one you want, override it** — the Add/Edit Task form has a **Type** dropdown right below Status. Leave it on "Auto" and it behaves exactly as described above; pick Client/Supplier/Vendor/Admin instead and that badge sticks regardless of what else is linked to the task. Handy for a task that has both a client and a supplier tagged on it (like a trip follow-up) where you'd rather it read as one specifically instead of whichever the automatic rule happens to pick.
...
- **Email** (the mail-merge tool) — quick canned messages: Welcome New Client, General Follow-Up, Trip Confirmation, Thank You & Referral, **Birthday Trip Planning**, or **Referral Credit Earned**. Check off one or more clients from the list (there's a **Select All**/**Select None** shortcut above it, and **Paying clients only**/**Subscribers only** checkboxes to narrow it down first), preview with their name(s)/email(s) filled in, send. Checking more than one sends a single email addressed to everyone you checked, with the greeting joining their names together ("Hi Alice and Bob,", "Hi Alice, Bob, and Carol," for three or more) — the same way the Referral Credit Earned template below already greeted multiple referrers at once.
  ...
  - **Referral Credit Earned** works differently from the others: pick one or more **referrers** (its own checkbox list, replacing the usual client list for just this template) and one or more **referred clients**. The email greets all the referrers together ("Hi Aaron and Alice,"), mentions all the referred people in the body, goes out to every referrer's email at once, and adds a row to each referrer's **Referral Credits** section (see [Individual Clients](#individual-clients)) when you click Send or Log as Note. You can't pick the same person as both a referrer and someone they referred — you'll get a warning instead of a nonsensical email.
...
**Facebook** — click **Connect Facebook** the first time (this same connection also powers Facebook Ads campaign management on the [Marketing](#marketing) page — one login covers both). Once connected, three sections: **Ads Performance** (spend/impressions/clicks/reach over the last 30 days), **Page Performance** (Page Likes, Page Views, Impressions, Reach, Post Engagements — the organic, non-ad-spend side of your Facebook Page), and **Instagram** (Followers, Posts, Reach, Views, Profile Views). Under Page Performance, a collapsed-by-default **Recent Posts** section (click ▸ to expand) lists your last 10 posts to the Page with the date, a snippet of the caption (click it to open the actual post), and its Likes/Comments/Shares — a quick look at which posts are actually landing, right next to the Page-wide totals. Instagram doesn't need its own separate login — Meta requires an Instagram Business/Creator account to be linked to a Facebook Page, so as long as your Page has one linked in Meta Business Suite, it shows up automatically once Facebook is connected. If it says no Instagram account is linked, that's a Meta Business Suite setting to fix, not something to reconnect here.
...
- **GA4**: Daily Users, Views & Sessions (30 Days), Top Pages, Traffic Sources, Device Category, Top Countries, and **AI Assistant Referrals (GEO)** — real visitors who clicked a link an AI assistant (ChatGPT, Perplexity, Gemini, Copilot, Claude, etc.) generated for them. There's no way to see how often an AI actually mentions or recommends your site in an answer — that's not something Google Analytics or anyone else exposes — so this instead shows the traffic you can actually measure: people who followed a link out of one of those tools straight to your site. It undercounts some real AI-driven visits too, since a few assistants (ChatGPT's mobile app, notably) don't pass along where the click came from, so those show up as ordinary Direct traffic instead.
