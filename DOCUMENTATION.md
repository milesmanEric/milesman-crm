...
- `booked.hotel` — an **array** of hotel-stay entries (a trip can have multiple hotels), each `{hotelBrand, confirmationNumber, checkInDate, nights, price, program, amount, notes}` — already had its own per-entry `price` before this round of categories got one; `notes` is per-entry too, not shared across hotel stays.
...
**`pts`** — every section above except Hotel (`booked.airline.pts`, `booked.cruise.pts`, `booked.carRental.pts`, `booked.toursPts`, `booked.themePark.pts`, `booked.train.pts`, `booked.bus.pts`) has its own "Points Used" block: a dynamic, add/remove-any-number-of-rows list of miles/points redemptions, each `{program, amount, cost}` — `program` is picked from a combined list of all airlines, hotels, and credit cards (a booking often mixes airline miles with a transferred credit-card-points balance), `amount` is points/miles used, `cost` is the cash paid alongside them (e.g. taxes/fees). Empty rows are filtered out on save, not stored as blanks. `booked.hotel` entries are deliberately excluded from this — each hotel stay already has its own single, finer-grained `program`/`amount` pair (see above), and giving Hotel a second, section-level points list alongside that per-stay one would just create a confusing duplicate tracking mechanism. (Before this became a dynamic list, `booked.airline.pts` was a fixed set of up to 3 blocks; the array shape is unchanged, so old trips load without migration.)
...
**Points deployed** (`tripBookedPoints()`) totals only the granular redemption entries recorded in each trip's Booked Trip Details — every section's `pts[].amount` (Airline, Cruise, Car Rental, Tours via `toursPts`, Theme Park, Train, Bus) plus every `booked.hotel[].amount` — not the simple top-level `pointsUsed` field on the trip's basic form, which is a separate, less precise number that isn't rolled into this total. The drill-down list is built the same way, one row per trip with `tripBookedPoints(t) > 0`.
...
The Add/Edit Trip modal covers flight, hotel, cruise, and generic per-brand loyalty-balance sections (shown/hidden based on which client is selected, mirroring their stored balances), plus a **Booked Trip Details** section — see §2's `booked` field for the full schema. It's organized into Airline / Hotel / Cruise / Car Rental / Tours / Theme Park / Train / Bus, each with its own confirmation number field; every section except Hotel additionally has its own "Points Used" block — a dynamic, add-any-number-of-rows list of miles/points redemptions (program + amount + cash cost), with the program dropdown combining every airline, hotel, and credit card option (§2's `pts`). Cruise Line selection cascades the Ship Name and Cabin Type dropdowns, reusing the same option lists as the quote builder (`getQuoteConfig`).
