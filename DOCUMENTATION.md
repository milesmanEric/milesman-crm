...
- `booked.cruise` — `confirmationNumber`, `cruiseLine`, `shipName`, `sailingDate`, `nights`, `departurePort`, `loyaltyNumber`, `price`, `notes`, `pts` (see below), and `cabins`: an **array** of cabin entries (a booking can cover more than one cabin on the same sailing), each `{cabinType, peopleInCabin}` — same dynamic add/remove pattern as `booked.hotel`/`booked.tours`; an old trip's single `cabinType`/`peopleInCabin` shape still loads fine as a one-entry list. Cabin Type options come from `getQuoteConfig(cruiseLine).cabinTypes` (the same config Email Quote uses), so every cabin row is rebuilt whenever Cruise Line changes.
...
- `booked.train` — `confirmationNumber`, `departureCity`, `arrivalCity`, `departureDate`, `returnDate`, `classOfService`, `price`, `notes`, `pts` (see below)
- `booked.bus` — same shape as `booked.train`: `confirmationNumber`, `departureCity`, `arrivalCity`, `departureDate`, `returnDate`, `classOfService`, `price`, `notes`, `pts` (see below)
...
- **Email Itinerary** (`openEmailItineraryModal`) — pick a client, pick one of their actual `db.trips` records, and it generates a styled HTML recap straight from whatever that trip has filled in: dates, passengers, booking reference, supplier, points/cash/total value, trip notes, plus a section per Booked Trip Details category — every section is conditional on the trip actually having that data, so e.g. a flight-only trip shows no Train/Bus/Car Rental/Tours/Theme Park/Cruise/Hotel boxes at all.
  ...
  - **Flight/Hotel/Cruise prefer Booked Trip Details over the trip's simpler top-level basic fields**, matching how Car Rental/Tours/Theme Park/Train/Bus already worked: if `booked.airline.segments` has entries, one "Flight Details" box is built per segment (Airline, Flight Number, Route, Depart Date, Depart/Arrival Time, Class of Service, plus the section-level Confirmation Number repeated on each); only falls back to the single `trip.airline`/`trip.origin`/`trip.destination`/`trip.flightClass`-based summary when there are no booked segments at all. Likewise `booked.hotel[]` entries each get their own "Hotel Details" box before falling back to `trip.hotel`/`trip.hotelNights`; `booked.cruise` fields (Cruise Line, Ship, Sailing Date, Nights, Departure Port, Confirmation Number, Loyalty Number, plus a row per `cabins[]` entry) enrich the single Cruise Details box, still falling back to `trip.cruiseLine`/`trip.shipName` if `booked.cruise` has nothing.
