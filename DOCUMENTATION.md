...
`referralCredits` — array of `{referredName, date, amount, used}`, shown/edited in a "Referral Credits" section on the client form (name, date, $ amount, a "Used" checkbox, delete button, "+ Add Referral Credit" for a manual row). Sending the "Referral Credit Earned" email template (§3 Email) auto-appends one row per referred person to each selected referrer's record — see §3 Email for the multi-referrer/referred mechanics.
...
### Email
A lightweight mail-merge tool: pick a client (or, for the referral template, multiple clients — see below), pick one of the canned templates (`TEMPLATES` in index.html — Welcome New Client, General Follow-Up, Trip Confirmation, Thank You & Referral, **Referral Credit Earned**), preview with placeholders substituted, then send via `mailto:`.

Most templates use `{{firstName}}`/`{{fullName}}`/`{{email}}` against a single selected client. **Referral Credit Earned** (`requiresReferral:true` on its `TEMPLATES` entry) is different: selecting it swaps "1. Select Client" into a multi-select checkbox list labeled "Referrer(s)", and reveals a second multi-select checkbox list, "Referred Client(s)". `fillTemplate()` accepts arrays for both, joining multiple names naturally ("Aaron and Alice" / "Aaron, Alice, and Bob") for `{{firstName}}`, and a separate `{{referredName}}` placeholder for the referred names — `toFirstLastNoMiddle()` drops any middle name from the referred mention specifically. Picking the same client as both a referrer and a referred client shows an inline warning instead of sending a nonsensical email. Selecting either "Open in Outlook" or "Log as Note" for this template also calls `addReferralCreditEntries()`, appending one `referralCredits` row (§2) per (referrer, referred person) pair — guarded so clicking both buttons for the same preview doesn't double-record.
